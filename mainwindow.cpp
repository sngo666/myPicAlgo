    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
  ...
  connect(ui->openLSB, SIGNAL(clicked(bool)), this,
          SLOT(SLOT_openLSBWidget()));
...
void MainWindow::DFT_clicked()
{
  ...
  if (fileName.isEmpty())
  {
    this->PrintLog("no Pic selected!", "red", "4");
    return;
  }
...
void MainWindow::SLOT_openLSBWidget()
{
  QDesktopWidget *desktop = QApplication::desktop();
  lsbWidget->show();
...
void MainWindow::createConnection()
{
  ...
  connect(this, SIGNAL(sendData(QString)), lsbWidget, SLOT(receiveFilePath(QString)));
}
...
void MainWindow::printMat(Mat &m_srcImage, QLabel *label)
{
  if (m_srcImage.data)
  {
...
void workThread::getFilePath(QString str)
{
  ...
  cout << "file received" << Qt::endl;
}

void workThread::startWork()
{
  emit workStart();
  cout << "start thread!" << Qt::endl;
  ...
  for (int i = 0; i < height; i++)
  {
    ...
    { //二维矩阵各点情况
      ...
      out_array[i][j] = (unsigned char)((sqrt(re * re + im * im))); //振幅
                                                                    //  =
      if (out_array[i][j] > 0xfe)
        out_array[i][j] = 0xff;
      else if (out_array[i][j] < 1)
        out_array[i][j] = 0;
    }
  }
  // printf("dft done\n");
